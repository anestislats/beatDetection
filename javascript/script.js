      ...
      request.onload = function() {
        context.decodeAudioData(request.response, function(buffer) {
          ...
          offlineContext.oncomplete = function(e) {
            ...
            text.innerHTML = '<div id="guess"><strong> JavaScript Algorithm </strong> for track <strong>' + track.name + '</strong> by ' +
              '<strong>' + track.artists[0].name + '</strong> calculated <strong>' + Math.round(top[0].tempo) + ' BPM </div>';


            var printENBPM = function(tempo) {
              text.innerHTML += '<div class="small">The tempo according to <strong>The Echo Nest API </strong> is <strong> ' +
                    tempo + ' BPM </strong> </div>';
            };
